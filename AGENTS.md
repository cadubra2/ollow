...
`test-agenda-dry-run.js`, `test-guards-internos.js`, `test-agendamento-bloco-mensagens.js`,
`test-transcricao.js`, `test-zapsign.js`, `test-notas-reuniao.js`, `test-notas-provedor-flags.js`,
...
- `test-notas-gemini-real.js <gmailMessageId...>` ou `--ultimos=N` — roda OpenAI **e** Gemini sobre a
  MESMA reunião real e imprime as duas notas lado a lado, com a contagem de campos preenchidos e de
  valores marcados como não localizados na fonte. **Só leitura** (não rotula e-mail, não escreve no
  CRM, `DB_PATH` descartável), mas custa crédito nos dois provedores e lê Gmail/Drive de produção,
  então fica fora do `npm test`. É o script que decide a troca de `NOTAS_IA_PROVEDOR`: teste
  determinístico com fixture não serviria — foi chamada real que pegou o JSON aninhado por seção e o
  caractere multi-byte corrompido, e o Gemini pode reproduzir o aninhamento de um jeito diferente.
...
src/
  ...
  gemini.js              tradução das mensagens do formato OpenAI para o Gemini e leitura da resposta
                         (puro, sem rede — quem fala HTTPS é geminiChat no index.js)
...
- **Quem ESCREVE o resumo e quem o ESTRUTURA são coisas diferentes — e só a segunda é configurável.**
  O resumo já chega pronto do Gemini, dentro do e-mail; a IA desta rotina só o converte nas 10 chaves
  de `CAMPOS`. `NOTAS_IA_PROVEDOR` (padrão `openai`) escolhe quem faz essa conversão:
  `geminiChat`/`openaiChat` têm a MESMA assinatura e devolvem a mesma string de JSON, então trocar
  não toca em nada do que vem depois (`validarExtracao`, `conferirAncoragem`, `montarTextoNota`, o
  bracket `POSTANDO → POSTADA`). Qual provedor respondeu fica gravado dentro da coluna `extracao`
  (`{ extracao, avisos, provedor }`) — invisível para o advogado, consultável na comparação.
- **O system prompt que o NotebookLM sugere: duas regras já existiam, uma faltava, e a estrutura
  quebraria a rotina.** Avaliado em 26/08/2026, a partir do texto que o próprio produto devolveu
  quando perguntado como reproduzir o comportamento dele. *Grounding* já é mais forte aqui do que
  lá: o prompt pede (regras 1, 2 e 4) **e** `conferirAncoragem` verifica em código, marcando inline
  todo número ausente da fonte — pedir ao modelo nunca foi a defesa principal. A parte de "aponte
  a lacuna em vez de assumir" foi **recusada**: contradiz a regra 4, que manda deixar o campo
  vazio justamente porque campo vazio é omitido da nota e "não mencionado" polui o briefing.
  *Atribuição* já estava na regra 6 (cliente x advogado) e ganhou o caso de vários integrantes da
  equipe pelo nome. **Limpeza de ruído era o buraco real** e virou a regra 10: metade da fonte é
  transcrição literal de voz (hesitação, retomada, palavra trocada pelo reconhecimento de fala) e
  nada no prompt dizia o que fazer com isso. A regra vem com trava: ruído não autoriza *completar*
  — nome ou número que a transcrição cortou fica cortado, nunca "corrigido" de memória, senão a
  instrução de limpar vira convite a alucinar. A **estrutura** sugerida (Visão Geral / Tópicos /
  Decisões / Ações, em Markdown) não é adotável: a rotina exige JSON plano com as 10 chaves de
  `CAMPOS` — prosa faz `JSON.parse` explodir ou cair em `vazia: true` → `ERRO_PERMANENTE`, e a nota
  nunca sai — e Markdown vira sujeira no `description` do Moskit, que é exibido cru. "Decisões" e
  "Próximos passos" como campos próprios seria mudança de formato, decisão do escritório (ver o
  vaivém de 20→21/08/2026), não do código.
- **NotebookLM não serve para isso, e não é questão de acesso.** Não há API pública self-serve (o
  produto virou *Gemini Notebook*; só existe preview Enterprise via Google Cloud, com allowlist), a
  API de notebook é stateful/assíncrona e devolve **prosa** onde esta rotina precisa de JSON com
  contrato, e — o decisivo — o valor do NotebookLM é ancorar num acervo grande demais para um prompt,
  enquanto aqui a fonte é **um e-mail + um Doc** que cabem inteiros no contexto. Dirigir o produto
  por automação de navegador quebraria em silêncio quando a sessão Google expirasse, que é a falha
  que `conferirSilencioNotas` existe para evitar. Avaliado e recusado em 26/08/2026.
- **Sem `responseSchema`, mesmo o Gemini suportando.** `responseMimeType: 'application/json'` basta.
  Fixar um esquema com as chaves de `CAMPOS` é o mesmo defeito já documentado em `honorarios`: campo
  fixo é convite a preencher, e perguntado sobre uma etapa que ninguém discutiu o modelo devolve um
  valor plausível. Quem julga o conteúdo continua sendo `validarExtracao`.
- **O teto de caracteres é do PROVEDOR, não da rotina.** `NOTAS_MAX_CHARS` (120k, ~30k tokens) é
  apertado pela janela do `gpt-4o-mini` e corta transcrição longa pelo começo;
  `NOTAS_MAX_CHARS_GEMINI` (800k, ~200k tokens) cabe com folga em 1M — é esse o ganho real da troca.
  Os dois são separados porque o fallback acontece justamente quando o Gemini falha: um teto único
  mandaria 800k chars para uma janela de 128k. Por isso `extrairComIaDeNotas` recebe `corpo`/
  `textoDoc` CRUS e recorta por provedor, devolvendo o `textoFonte` que REALMENTE foi enviado —
  `validarExtracao` tem que conferir a ancoragem contra o texto que o modelo viu, não contra um maior.
- **O fallback nunca é silencioso.** Gemini falhando cai para a OpenAI (para não perder a reunião),
  mas com `console.error` + Telegram citando o modelo. Chave errada ou modelo inexistente mascarados
  para sempre seriam exatamente o padrão de falha que o resto desta rotina combate.
- **`NOTAS_IA_PROVEDOR` é fixado em `openai` por `rodar-testes.js`**, pelo mesmo motivo de `TZ`: a
  suíte não pode mudar de comportamento porque o dev tem uma variável no shell. Quem exercita o
  Gemini é `test-notas-provedor-flags.js`, que se relança como filho (a flag é `const` lida no
  require, mesmo motivo de `test-cliente-retorno-flags.js` existir) nos três modos: `openai`,
  `gemini` e `gemini-falha`.
